...
### Changed
- `waitForPort` 同时检测进程存活，进程退出时立即返回（不再空等 10 秒超时）
- 启动失败时自动内联展示日志前 20 行，无需手动打开日志文件
- 区分"进程已退出"和"端口超时"两种失败提示
...
### Changed
- 提取 `proxyStatus()` 函数，统一代理状态行格式化（去重 4 处重复代码）
- `proxyStatus` 的 `label` 参数收窄为联合类型，防止传入非法值
- 状态检查增加 `state.port !== null` 守卫，消除非空断言
...
### Changed
- `proxy status` 改名为 `proxy use`（status 保留为别名）
