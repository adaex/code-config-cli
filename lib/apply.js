...
const { readConfigSettings, hasProxy, extractLocalPort } = require('./configs')
const { hasDrift, createBackup } = require('./backup')
const { startProxy, stopProxy, waitForPort } = require('./proxy')
const { info, warn, error, success, dim, dryRun, step } = require('./logger')
const { DIM, RESET, GREEN, CYAN } = require('./logger')
...
async function applyConfig(cccDir, configName, isDryRun) {
  ...
  if (!newSettings) {
    error(`未找到配置 "${configName}" 的 settings.json`)
    process.exit(1)
  }
  ...
  if (localPort && hasProxy(cccDir, configName)) {
    ...
    if (isDryRun) {
      ...
      const result = startProxy(cccDir, configName, effectivePort, ts)
  ...
  if (isDryRun) {
    ...
    // 展示配置摘要：url / 模型
    const env = newSettings.env || {}
    const url = env.ANTHROPIC_BASE_URL || ''
    const model = env.ANTHROPIC_DEFAULT_SONNET_MODEL || newSettings.model || ''
    const parts = []
